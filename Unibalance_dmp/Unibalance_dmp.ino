...
//      dbgClient.println(angle);
  ...
  if(dt_p >= pidPeriod){
    double err = angle - target - tilt;
    errSum = errSum / 3 + prevErr * dt_p;
    dErr = (err - prevErr) / dt_p;
    output = (double)kp/100 * err + (double)ki/100 * errSum + (double)kd/100 * dErr;
    prevErr = err;
    lastPid = now;    

    double outputL = output - steer;
    double outputR = output + steer;
    double absOutputL = abs(outputL);
    double absOutputR = abs(outputR);

    unsigned int finalOutL = map(min(1000, absOutputL), 0, 1000, 0, maxV);
    unsigned int finalOutR = map(min(1000, absOutputR), 0, 1000, 0, maxV);
    
    if (outputL > 0) {
      analogWrite(MOT_L_F, finalOutL);
      analogWrite(MOT_L_B, 0);      
    } else {
      analogWrite(MOT_L_F, 0);
      analogWrite(MOT_L_B, finalOutL);
    }
    if (outputR > 0) {
      analogWrite(MOT_R_F, finalOutR);
      analogWrite(MOT_R_B, 0);
    } else {
      analogWrite(MOT_R_F, 0);
      analogWrite(MOT_R_B, finalOutR);
    }
